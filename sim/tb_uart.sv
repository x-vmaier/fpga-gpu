...
program tb_uart ();
    parameter realtime BAUD_PERIOD  = 1_000_000_000.0 / 8_000_000.0;
    ...
    `TB_TEST_PART("Random receive test")
        ...
        for (int i = 0; i < 100; i++) begin
            ...
            fork
                ...
                `wait_for_posedge(TB.u_gpu.uart_if.rx_valid, FRAME_PERIOD)
            join
            `Check(TB.u_gpu.uart_if.rx_data == test_byte,
                ("Byte %0d: expected 0x%02X got 0x%02X", i, test_byte, TB.u_gpu.uart_if.rx_data))
        end
    ...
    `TB_TEST_PART("Random transmit test")
        for (int i = 0; i < 100; i++) begin
            ...
            fork
                wait(TB.u_gpu.uart_if.tx_ready == 1'b1);
                begin
                    #(FRAME_PERIOD);
            ...
            force TB.u_gpu.uart_if.tx_data = test_byte;
            force TB.u_gpu.uart_if.tx_valid = 1'b1;
            fork
                wait(TB.u_gpu.uart_if.tx_ready == 1'b0);
                begin
                    #(FRAME_PERIOD);
            ...
            join_any
            disable fork;
            force TB.u_gpu.uart_if.tx_valid = 1'b0;
        ...
        release TB.u_gpu.uart_if.tx_data;
        release TB.u_gpu.uart_if.tx_valid;
